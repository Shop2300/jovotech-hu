...
export default function CartPage() {
  ...
  return (
    <main className="min-h-screen bg-white">
      {/* Progress Bar */}
      <div className="py-4 md:py-6 bg-white z-10">
        <div className="max-w-screen-2xl mx-auto px-4 md:px-6">
          <div className="flex items-center justify-center">
            <div className="flex items-center w-full max-w-3xl">
              {/* Step 1: Cart */}
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-4 md:gap-8">
          ...
          <div className="lg:col-span-2">
            <div className="space-y-4">
              {items.map((item) => (
                <div key={`${item.id}-${item.variantId || 'default'}`} className="border border-gray-200 rounded-lg p-4 md:border-0 md:border-b md:rounded-none md:p-0 md:py-4 last:border-b-0">
                  <div className="flex flex-col sm:flex-row gap-4">
                    ...
                    <Link 
                      href={item.categorySlug && item.productSlug 
                        ...
                        : `/products/${item.id}`}
                      className="w-full sm:w-24 h-32 sm:h-24 bg-gray-200 rounded-md flex-shrink-0 overflow-hidden hover:opacity-80 transition-opacity touch-manipulation"
                    >
                      {item.image ? (
                        <img 
                          src={item.image} 
                    ...
                    <div className="flex-grow flex flex-col sm:flex-row sm:gap-4">
                      {/* Product Info */}
                      <div className="flex-grow">
                        ...
                        <Link 
                          href={item.categorySlug && item.productSlug 
                            ? `/${item.categorySlug}/${item.productSlug}` 
                            : `/products/${item.id}`}
                          className="text-base text-[#131921] hover:underline transition-all inline-block touch-manipulation"
                        >
                        ...
                        {item.variantName && (
                          <div className="flex items-center gap-2 mt-1">
                            ...
                            <span className="text-gray-600 text-sm">{item.variantName}</span>
                          </div>
                        )}
                        {/* Availability */}
                        ...
                        {/* Mobile: Price for small screens */}
                        <div className="mt-2 sm:hidden">
                          {item.regularPrice && item.regularPrice > item.price ? (
                            <div className="flex items-center gap-2">
                              ...
                              <span className="inline-block bg-red-600 text-white px-1.5 py-0.5 rounded text-xs font-bold">
                        ...
                        <div className="flex items-center justify-between mt-3 gap-4">
                          ...
                          <div className="flex items-center">
                            <div className="flex items-center border border-gray-300 rounded-md overflow-hidden">
                              ...
                              <input
                                ...
                                className="w-14 md:w-12 h-10 md:h-8 text-center border-x border-gray-300 focus:outline-none text-[#131921] [appearance:textfield] [&::-webkit-outer-spin-button]:appearance-none [&::-webkit-inner-spin-button]:appearance-none"
                                maxLength={5}
                              />
                              <button
                                onClick={() => updateQuantity(item.id, item.quantity + 1, item.variantId)}
                          ...
                          <button
                            ...
                            className="text-gray-400 hover:text-gray-600 transition p-2 touch-manipulation sm:hidden"
                      ...
                      <div className="text-right hidden sm:block">
                        ...
                        <button
                          onClick={() => removeItem(item.id, item.variantId)}
                          className="text-gray-400 hover:text-gray-600 mt-2 transition p-2 touch-manipulation"
                          aria-label="Usuń produkt"
                        >
                          <Trash2 size={20} />
                        </button>
          ...
          {/* Order Summary - Mobile: Fixed at bottom, Desktop: Sticky sidebar */}
          <div className="lg:col-span-1 fixed bottom-0 left-0 right-0 lg:relative lg:bottom-auto lg:left-auto lg:right-auto bg-white lg:bg-transparent z-20">
                            <div className="bg-white rounded-t-2xl lg:rounded-lg shadow-[0_-4px_6px_-1px_rgba(0,0,0,0.1)] lg:shadow-md p-4 md:p-6 lg:sticky lg:top-6">
              <h2 className="text-lg md:text-xl font-semibold mb-4 text-[#131921] hidden lg:block">Podsumowanie zamówienia</h2>
              
              {/* Mobile Summary - Collapsed View */}
              <div className="lg:hidden">
                <div className="flex justify-between items-center">
                  <div>
                    <p className="text-sm text-gray-600">Razem ({items.length} {items.length === 1 ? 'produkt' : items.length < 5 ? 'produkty' : 'produktów'})</p>
                    <p className="text-xl font-bold text-[#8bc34a]">{formatPrice(totalPrice)}</p>
                  </div>
                  <button
                    onClick={handleCheckout}
                    className="bg-[#8bc34a] text-white px-6 py-3 rounded-lg hover:bg-[#7cb342] transition touch-manipulation min-h-[48px]"
                  >
                    Przejdź do kasy
                  </button>
                </div>

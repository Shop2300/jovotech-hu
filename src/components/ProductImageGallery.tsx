...
export function ProductImageGallery({ images, productName }: ProductImageGalleryProps) {
  ...
  // Fullscreen modal for mobile/desktop zoom
  const FullscreenModal = () => (
    <div className="fixed inset-0 z-50 bg-black flex items-center justify-center">
      ...
      <div className="relative w-full h-full flex items-center justify-center p-4">
        ...
        {/* Navigation in fullscreen (bigger on desktop) */}
        {sortedImages.length > 1 && (
          <>
            <button
              ...
              style={{ width: isMobile ? 44 : 56, height: isMobile ? 44 : 56 }}
              aria-label="Előző kép"
            >
              <ChevronLeft size={isMobile ? 24 : 34} className="text-gray-700" />
            </button>
            <button
              onClick={goToNext}
            ...
            >
              <ChevronRight size={isMobile ? 24 : 34} className="text-gray-700" />
            </button>
          </>
        )}
    ...
    <>
      <div className="space-y-4">
        ...
        <div className="relative group">
            ...
            <>
              <button
                ...
                className={`absolute left-2 top-1/2 -translate-y-1/2 z-10 bg-white shadow-lg rounded-full hover:bg-gray-100 transition touch-manipulation flex items-center justify-center ${
                  isMobile ? 'opacity-90 border border-gray-300' : 'opacity-0 group-hover:opacity-100 p-3'
                }`}
                style={isMobile ? { width: '44px', height: '44px' } : {}}
                aria-label="Előző kép"
              >
                <ChevronLeft size={isMobile ? 24 : 36} className="text-gray-700" />
              </button>
              <button
                onClick={(e) => {
                  e.stopPropagation();
                ...
                aria-label="Következő kép"
              >
                <ChevronRight size={isMobile ? 24 : 36} className="text-gray-700" />
              </button>
            </>
          )}
          ...
          {sortedImages.length > 1 && (
            <div
              className={`absolute bottom-4 left-1/2 -translate-x-1/2 bg-white shadow-md px-3 py-1.5 rounded-full text-sm text-gray-700 font-medium ${
              ...
              {sortedImages.map((_, index) => (
                <div
                  ...
                  className={`w-2 h-2 rounded-full transition-all ${
                    index === selectedIndex ? 'bg-[#8bc34a] w-6' : 'bg-gray-300'
                  }`}
                />
              ))}
            </div>
        ...
        {sortedImages.length > 1 && (
          <div
            ref={thumbnailsRef}
            className={`${isMobile ? 'flex gap-2 overflow-x-auto scrollbar-hide pb-2 -mx-4 px-4' : 'grid grid-cols-6 gap-2'}`}
          ...
          >
            {sortedImages.map((image, index) => (
              <button
                ...
                className={`${isMobile ? 'flex-shrink-0 w-20 h-20' : 'aspect-square'} rounded-lg overflow-hidden border-2 transition touch-manipulation ${
                  index === selectedIndex ? 'border-[#8bc34a]' : 'border-gray-200 hover:border-gray-400'
                }`}
              >
                <img
                  src={image.url}
